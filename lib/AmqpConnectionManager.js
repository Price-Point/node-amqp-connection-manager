...
class AmqpConnectionManager extends _events.EventEmitter {
  ...
  _connect() {
    ...
    return Promise.resolve().then(() => {
      ...
      const realUrlString = _url.default.format(amqpUrl);

      _winston.default.error(realUrlString);

      _winston.default.error(JSON.stringify(connectionOptions));

      return _amqplib.default.connect(realUrlString, connectionOptions).then(connection => {
      ...
      const amqpURL = _url.default.parse(this._urls[0]);

      const formattedURL = _url.default.format(amqpURL);
